...
import { PHYSICS, SPEED, CAMERA, TUBE, SHIP, RENDER, PICKUP, TREASURE, HAZARD, BACKGROUND, ENERGY, LIVES, SCORE, GUN, ENEMY, PROJECTILE, INPUT, AUDIO } from './config'
...
audio.playMusic('menu') // queued until the context unlocks on the first gesture
...
let wasOver = false // detects the run-ending edge once in render (sting + music)
...
startLoop(
  ...
  (frameDt) => {
    ...
    if (game.over && !wasOver) {
      audio.play('gameover')
      audio.playMusic('menu') // back to the menu theme on the game-over screen
    }
    wasOver = game.over
...
;(window as unknown as { WH: unknown }).WH = {
  ...
  config: { PHYSICS, SPEED, CAMERA, TUBE, SHIP, RENDER, PICKUP, TREASURE, HAZARD, BACKGROUND, ENERGY, LIVES, SCORE, GUN, ENEMY, PROJECTILE, AUDIO },
}
