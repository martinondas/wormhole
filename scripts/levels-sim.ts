...
// Exercises src/flight.ts directly (no DOM): level boundaries by distance, per-
...
import { createFlight } from '../src/flight'
import { SPEED, PHYSICS, FLIGHT, LEVELS, PICKUP, HAZARD } from '../src/config'
...
// cumulative distance where level L begins
...
// targetSpeed/level/multiplier at a distance (via update)
...
// ---- A) level boundaries by distance ----
;(() => {
...
;(() => {
  ...
  const f = createFlight()
  f.update(cumStart(1) + 50) // level 1, slow section
  ...
  const g = createFlight()
  g.update(cumStart(1) + 50)
...
// ---- F) tier/level lookups used by tube coloring + field scaling ----
;(() => {
  const f = createFlight()
  ...
  // a ring a level ahead reports the next level (rings span current+next level)
  check('F4 levelAt across boundary', f.levelAt(cumStart(1) + 10) === 1)
  check('F5 tierIndexAt L1 slow', f.tierIndexAt(cumStart(1) + 10) === 0)
})()

// ---- G) pinned modes: no level progression, fixed tier ----
;(() => {
  const f = createFlight()
  f.mode = 'slow'
  f.update(99999)
...
;(() => {
  const f = createFlight()
  check('H1 speedRatio = 1 at level 1', approx(f.speedRatioAt(100), 1))
  check('H2 speedRatio = 1 at level 1 (any tier)', approx(f.speedRatioAt(1000), 1))
  ...
  check('H3 speedRatio L5 normal tier = (52+20)/52', approx(f.speedRatioAt(dNormalL5), (base[1]! + 4 * inc) / base[1]!), `${f.speedRatioAt(dNormalL5)}`)
  ...
  const tierSpeedAt = (lvl: number, tier: number): number => base[tier]! + lvl * inc
  const orbRate = (lvl: number, dist: number): number => {
    const mult = LEVELS.TABLE[Math.min(lvl, LEVELS.TABLE.length - 1)]!.orbSpacingMult
    return tierSpeedAt(lvl, 1) / (PICKUP.SPAWN_SPACING * mult * f.speedRatioAt(dist))
  }
  const bombRate = (lvl: number, dist: number): number => {
    const mult = LEVELS.TABLE[Math.min(lvl, LEVELS.TABLE.length - 1)]!.bombSpacingMult
    return tierSpeedAt(lvl, 1) / (HAZARD.SPAWN_SPACING * mult * f.speedRatioAt(dist))
  }
  const orbL1 = orbRate(0, 400) // L1 normal section
  const orbL5 = orbRate(4, dNormalL5)
  check('H4 orb rate FALLS L1 -> L5 (energy harder, fix works)', orbL5 < orbL1, `L1 ${orbL1.toFixed(3)}/s -> L5 ${orbL5.toFixed(3)}/s`)
  const bombL1 = bombRate(0, 400)
  const bombL5 = bombRate(4, dNormalL5)
